...
void print_set_time(unsigned char hour, unsigned char min, unsigned char sec) {
    if (change_time_flag == 1) {
        if (hour_delay++ <= 100) {
            clcd_putch((hour / 10) + 48, (0xC0 + (0)));
            clcd_putch((hour % 10) + 48, (0xC0 + (1)));
...
void set_time(void) {
    ...
    if (key == 11) {
        write_ds1307(0x02, ((hour_var/10)<<4) | (hour_var%10));
        write_ds1307(0x01, ((min_var/10)<<4) | (min_var%10));
        write_ds1307(0x00, ((sec_var/10)<<4) | (sec_var%10));
        set_time_once = 0;
        state = e_dashboard;
